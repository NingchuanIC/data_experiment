...
def parse_args() -> argparse.Namespace:
	...
	parser = argparse.ArgumentParser(
		description="Train a ResidualGAT using prebuilt graph dataset files."
	)
	parser.add_argument(
		"--graph-data-dir",
		type=Path,
	...
	parser.add_argument(
		...
		type=int,
		default=32,
		help="Hidden dimension for ResidualGAT.",
	)
	parser.add_argument(
		"--heads",
		type=int,
	...
	parser.add_argument(
		...
		type=float,
		default=0.0,
		help="Dropout probability after first GAT layer.",
	)
	parser.add_argument(
	...
	parser.add_argument(
		"--save-model-path",
		type=Path,
		default=repo_root / "ResidualGAT_news_graph_feat_6" / "residual_gat_model.pt",
		help="Path to save trained model checkpoint.",
	)
	parser.add_argument(
		"--output-log",
		type=Path,
		default=repo_root / "ResidualGAT_news_graph_feat_6" / "residual_gat_train_log.txt",
...
def train(
	...
	for epoch in range(1, epochs + 1):
		...
		for t in range(n_steps):
			...
			loss = negative_corr_loss(pred[mask], y[mask])
		...
		line = (
			f"epoch={epoch:03d} ic_loss={avg_loss:.8f} | val_ic={current_val_ic:.8f} | "
...
def save_outputs(
	...
	ckpt = {
		...
		"model": f"ResidualGAT({len(feature_cols)}->32->1)",
	}
...
def main() -> None:
	...
	result_lines = [
		"ResidualGAT Regression (fixed year split, prebuilt graph dataset, IC loss)",
