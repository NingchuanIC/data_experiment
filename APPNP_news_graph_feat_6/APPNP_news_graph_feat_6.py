...
def parse_args() -> argparse.Namespace:
	...
	parser = argparse.ArgumentParser(
		description="Train an APPNPModel using prebuilt graph dataset files."
	)
	parser.add_argument(
		"--graph-data-dir",
		type=Path,
	...
	parser.add_argument(
		...
		type=int,
		default=32,
		help="Hidden dimension for APPNPModel.",
	)
	parser.add_argument(
	...
	parser.add_argument(
		"--save-model-path",
		type=Path,
		default=repo_root / "APPNP_news_graph_feat_6" / "appnp_model.pt",
		help="Path to save trained model checkpoint.",
	)
	parser.add_argument(
		"--output-log",
		type=Path,
		default=repo_root / "APPNP_news_graph_feat_6" / "appnp_train_log.txt",
...
def save_outputs(
	...
	ckpt = {
		...
		"model": f"APPNPModel({len(feature_cols)}->32->1)",
	}
...
def main() -> None:
	...
	model = APPNPModel(in_dim=len(feature_cols), hidden_dim=args.hidden_dim)
	...
	result_lines = [
		"APPNP Regression (fixed year split, prebuilt graph dataset, IC loss)",
