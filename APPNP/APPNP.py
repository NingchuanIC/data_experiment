...
def parse_args() -> argparse.Namespace:
	...
	parser.add_argument(
		...
		type=int,
		default=4,
		help="Deprecated: number of attention heads (unused for APPNP).",
	)
	parser.add_argument(
		"--dropout",
		type=float,
	...
	parser.add_argument(
		"--save-model-path",
		type=Path,
		default=repo_root / "ResidualGAT" / "residual_gat_model.pt",
		help="Path to save trained model checkpoint.",
	)
	parser.add_argument(
		"--output-log",
		type=Path,
		default=repo_root / "ResidualGAT" / "residual_gat_train_log.txt",
...
def main() -> None:
	...
	result_lines = [
		"APPNP Regression (fixed year split, prebuilt graph dataset)",
		...
		f"K: {getattr(model.prop, 'K', 'N/A')}",
		f"alpha: {getattr(model.prop, 'alpha', 'N/A')}",
