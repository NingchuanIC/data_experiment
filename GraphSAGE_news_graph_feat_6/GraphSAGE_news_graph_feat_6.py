...
def parse_args() -> argparse.Namespace:
	...
	parser.add_argument(
		"--save-model-path",
		type=Path,
		default=repo_root / "GraphSAGE_news_graph_feat_6" / "graphsage_model.pt",
		help="Path to save trained model checkpoint.",
	)
	parser.add_argument(
		"--output-log",
		type=Path,
		default=repo_root / "GraphSAGE_news_graph_feat_6" / "graphsage_train_log.txt",
...
def save_outputs(
	...
	ckpt = {
		...
		"model": f"GraphSAGE({len(feature_cols)}->32->1)",
	}
...
def main() -> None:
	...
	model = GraphSAGE(in_dim=len(feature_cols), hidden_dim=args.hidden_dim)
	...
	result_lines = [
		"GraphSAGE Regression (fixed year split, prebuilt graph dataset, IC loss)",
