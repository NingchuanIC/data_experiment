...
def _compute_features(df: pd.DataFrame) -> pd.DataFrame:
	...
	out[target_col] = out["close"].shift(-1) / out["close"] - 1
...
def build_all_features(daily_dir: Path, basic_dir: Path) -> pd.DataFrame:
	...
	news_root = data_root / "news_score_500"
...
def main() -> None:
	...
	basic_dir = data_root / "daily_basic"
	out_root = data_root / "build_feature_news_ret1" / "dataset_by_month"

	if not daily_dir.exists():
		raise FileNotFoundError(f"daily directory not found: {daily_dir}")
	if not basic_dir.exists():
		raise FileNotFoundError(f"daily_basic directory not found: {basic_dir}")
	news_dir = data_root / "news_score_500"
	if not news_dir.exists():
		raise FileNotFoundError(f"news_score_500 directory not found: {news_dir}")
