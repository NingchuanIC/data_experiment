...
DEFAULT_INPUT_PATH = Path(
    r"D:\imperial_homework\third_year\FYP\GCN\data\build_feature_news_graph\dataset_by_month\2019\2019_01.parquet"
)
DEFAULT_OUTPUT_DIR = Path(
    r"D:\imperial_homework\third_year\FYP\GCN\data\build_feature_news_graph"
)
...
def main() -> None:
    ...
    lines = []
    lines.append(f"Loaded: {target_path}")
    lines.append(f"Shape: {df.shape}")
    lines.append("")
    lines.append(f"=== Head {args.n} ===")
    lines.append(df.head(args.n).to_string())
    lines.append("")
    lines.append(f"=== Tail {args.n} ===")
    lines.append(df.tail(args.n).to_string())

    output_text = "\n".join(lines)
    print(output_text)

    DEFAULT_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    out_path = DEFAULT_OUTPUT_DIR / f"{target_path.stem}_head_tail.txt"
