...
def _compute_days_since_last_news(trade_dates: pd.Series, has_news: pd.Series) -> pd.Series:
	...
	return pd.Series(days, index=trade_dates.index).fillna(MAX_NEWS_GAP).astype(np.int32)
...
def _rolling_news_features(group: pd.DataFrame) -> pd.DataFrame:
	...
	g["news_sent_dev_1d"] = np.where(
		g["news_cnt_1d"] > 0,
		g["news_sent_sum_1d"] / g["news_cnt_1d"] - NEWS_SENTIMENT_BASE,
		0.0,
	)
	g["news_sent_dev_3d"] = np.where(
		g["news_cnt_3d"] > 0,
		g["news_sent_sum_3d"] / g["news_cnt_3d"] - NEWS_SENTIMENT_BASE,
		0.0,
	)
	g["news_sent_dev_7d"] = np.where(
		g["news_cnt_7d"] > 0,
		g["news_sent_sum_7d"] / g["news_cnt_7d"] - NEWS_SENTIMENT_BASE,
		0.0,
	)

	g["news_sent_std_7d"] = g["news_sent_dev_1d"].rolling(7, min_periods=1).std(ddof=0)

	g["has_news_1d"] = (g["news_cnt_1d"] > 0).astype(np.int8)
	g["has_news_3d"] = (g["news_cnt_3d"] > 0).astype(np.int8)
	g["has_news_7d"] = (g["news_cnt_7d"] > 0).astype(np.int8)
	...
	g["news_cnt_1d_log"] = np.log1p(g["news_cnt_1d"].astype(np.float32))
	g["news_cnt_3d_log"] = np.log1p(g["news_cnt_3d"].astype(np.float32))
	g["news_cnt_7d_log"] = np.log1p(g["news_cnt_7d"].astype(np.float32))
	g["news_risk_cnt_7d_log"] = np.log1p(g["news_risk_cnt_7d"].astype(np.float32))

	has_news = g["news_cnt_1d"] > 0
	g["days_since_last_news"] = _compute_days_since_last_news(g["trade_date"], has_news).clip(upper=MAX_NEWS_GAP)
	g["news_freshness"] = np.exp(-g["days_since_last_news"] / NEWS_FRESHNESS_TAU)
...
def _build_news_features(news_root: Path, stock_panel: pd.DataFrame) -> pd.DataFrame:
	...
	rolled["has_news_1d"] = rolled["has_news_1d"].fillna(0).astype(np.int8)
	rolled["has_news_3d"] = rolled["has_news_3d"].fillna(0).astype(np.int8)
	rolled["has_news_7d"] = rolled["has_news_7d"].fillna(0).astype(np.int8)
	rolled["news_cnt_1d_log"] = rolled["news_cnt_1d_log"].fillna(0.0)
	rolled["news_cnt_3d_log"] = rolled["news_cnt_3d_log"].fillna(0.0)
	rolled["news_cnt_7d_log"] = rolled["news_cnt_7d_log"].fillna(0.0)
	rolled["news_sent_dev_1d"] = rolled["news_sent_dev_1d"].fillna(0.0)
	rolled["news_sent_dev_3d"] = rolled["news_sent_dev_3d"].fillna(0.0)
	rolled["news_sent_dev_7d"] = rolled["news_sent_dev_7d"].fillna(0.0)
	rolled["news_sent_std_7d"] = rolled["news_sent_std_7d"].fillna(0.0)
	rolled["news_risk_cnt_7d_log"] = rolled["news_risk_cnt_7d_log"].fillna(0.0)
	rolled["news_freshness"] = rolled["news_freshness"].fillna(np.exp(-MAX_NEWS_GAP / NEWS_FRESHNESS_TAU))
...
def build_all_features(daily_dir: Path, basic_dir: Path) -> pd.DataFrame:
	...
	news_root = repo_root / "news_score_500"
...
def main() -> None:
	...
	basic_dir = repo_root / "daily_basic"
	out_root = repo_root / "build_feature_news_graph" / "dataset_by_month"
